...
const VideoExtension = Node.create({
  ...
  addAttributes() {
    return {
      ...
      class: {
        default:
          "w-full h-auto rounded-lg shadow-md aspect-video my-6 bg-black",
      },
    };
  },
...
const PendingMediaExtension = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    ...
    return [
      "div",
      mergeAttributes(HTMLAttributes, { class: "smri-media-pending" }),
    ];
  },
  ...
  addNodeView() {
    return ({ node }) => {
      ...
      wrapper.className =
        "smri-media-pending p-6 border-2 border-dashed border-cyan-200 rounded-xl bg-cyan-50 my-6 text-center select-none";
    ...
    <div className="border-b border-gray-200 p-2 flex flex-wrap gap-1 bg-gray-50 sticky top-0 z-10 items-center">
      ...
      <div className="flex items-center gap-1 border-r border-gray-300 pr-2 mr-1">
        ...
        <button
          ...
          className={`p-1.5 rounded hover:bg-gray-200 ${editor.isActive("underline") ? "bg-gray-200 text-black" : "text-gray-600"}`}
          title="Underline"
        >
      ...
      <div className="flex items-center gap-1 border-r border-gray-300 pr-2 mr-1">
        <select
          ...
          className="h-8 text-xs border border-gray-200 rounded px-1 bg-white w-24"
        ...
        <select
          ...
          className="h-8 text-xs border border-gray-200 rounded px-1 bg-white w-16"
        ...
        <div className="relative flex items-center">
          <input
            ...
            className="w-8 h-8 p-0.5 border-none bg-transparent cursor-pointer"
            title="Text Color"
          />
        </div>
        <button
...
const TipTapEditor = forwardRef(
  (
    ...
    const [isLoadingS3, setIsLoadingS3] = useState(
      () => typeof content === "string" && content.startsWith("s3-content://"),
    );
      ...
      if (typeof content === "string" && content.startsWith("s3-content://")) {
        ...
        getPresignedContentUrl(content)
          ...
          .catch((err) => {
            ...
            setFetchedContent(
              "<p>Error loading lesson content. Please try again.</p>",
            );
            setIsLoadingS3(false);
          });
      } else {
    ...
    const editor = useEditor({
      extensions: [
        ...
        Table.configure({
          ...
          HTMLAttributes: {
            class:
              "border-collapse table-auto w-full my-4 border border-gray-300",
          },
        }),
        TableRow,
      ...
      editorProps: {
        attributes: {
          class:
            "prose prose-sm sm:prose-base focus:outline-none max-w-none min-h-[300px] p-4",
        },
      },
    });
    ...
    useEffect(() => {
      ...
      const checkPendingMedia = async () => {
        ...
        const pendingElements = editor.view.dom.querySelectorAll(
          ".smri-media-pending",
        );
          ...
          try {
            ...
            if (
              ...
              const placeholder = doc.querySelector(
                `.smri-media-pending[data-smri-media-id="${mediaId}"]`,
              );
              ...
              if (placeholder) {
                ...
                const regex = new RegExp(
                  `<div[^>]*data-smri-media-id="${mediaId}"[^>]*>.*?</div>`,
                  "s",
                );
