...
const MEDIA_SERVICE_URL = process.env.NEXT_PUBLIC_MEDIA_API_URL || "https://34gex7izc1.execute-api.us-east-1.amazonaws.com";
...
export const getUploadUrl = async ({
  ...
  // Service has /media/av/upload-url (for video/audio) and /media/images/upload-url (for image)
  const endpoint = (mediaType === 'image') 
    ? `${MEDIA_SERVICE_URL}/media/images/upload-url`
    : `${MEDIA_SERVICE_URL}/media/av/upload-url`;
  ...
  if (!response.ok) {
    ...
    throw new Error(errorData.detail || `Failed to get upload URL: ${response.statusText}`);
  }
...
export const getMediaItem = async (mediaId, mediaType) => {
  ...
  const endpoint = (mediaType === 'image')
    ? `${MEDIA_SERVICE_URL}/media/images/${mediaId}`
    : `${MEDIA_SERVICE_URL}/media/av/${mediaId}`;
